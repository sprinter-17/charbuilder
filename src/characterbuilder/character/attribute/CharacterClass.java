...
public enum CharacterClass implements Attribute {
    ...
    FIGHTER(10, 5, MARTIAL_ARCHETYPE, STRENGTH, CONSTITUTION,
        ...
        gen.level(1).addAttributes(ALL_ARMOUR, ALL_MELEE_WEAPONS, ALL_RANGED_WEAPONS, SECOND_WIND);
        gen.level(1).addChoice(() -> new MultiChoice(2, new AttributeChoice("Skill",
    ...
    WIZARD(6, 4, ARCANE_TRADITION, INTELLIGENCE, WISDOM,
        Arrays.asList(INTELLIGENCE, DEXTERITY, CONSTITUTION), (ChoiceGenerator gen) -> {
    ...
    static {
        for (int charLev = 1; charLev <= 20; charLev++) {
            for (int spellLev = 0; spellLev < SPELLS[charLev].length; spellLev++) {
                int count = SPELLS[charLev][spellLev];
                if (spellLev < SPELLS[charLev - 1].length)
                    count -= SPELLS[charLev - 1][spellLev];
                if (count > 0) {
                    addSpellChoice(CLERIC, charLev, spellLev, count);
                    addSpellChoice(WIZARD, charLev, spellLev, count);
                }
            }
        }
    }

    private static void addSpellChoice(CharacterClass characterClass,
        int charaterLevel, int spellLevel, int count) {
        characterClass.generator.level(charaterLevel)
            .addChoice(spellChoice(count, characterClass, spellLevel));
    }
    ...
    private static Choice spellMasteryChoice(String name, int level) {
        return new Choice() {
            ...
            public void makeChoice(Character character, ChoiceSelector selector) {
                selector.getAttribute(Spell.spells(character.getAttribute(CHARACTER_CLASS), level)
                    .filter(sp
