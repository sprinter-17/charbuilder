...
public class Character {
	...
	protected static final int[] XP_LEVELS = { 0, 300, 900, 2700, 6500, 14000, 23000, 34000, 48000, 64000, 85000,
	...
	public boolean hasChoices() {
		return getAllowedChoices().findAny().isPresent();
	}

	public Stream<OptionChoice> getAllowedChoices() {
		assert choices != null;
	...
	public boolean hasChoice(String name) {
		assert choices != null;
		return getAllowedChoices().map(OptionChoice::getName).anyMatch(name::equals);
	}

	public boolean hasChoice(String name, int count) {
		assert choices != null;
		return getAllowedChoices().map(OptionChoice::toString).anyMatch(String.format("%s (x%d)", name, count)::equals);
	}
	...
	public SpellCasting getSpellCasting(String name) {
		return getAttributes(AttributeType.SPELLCASTING, SpellCasting.class).filter(sc -> sc.hasName(name)).findAny()
				.orElseThrow(() -> new IllegalArgumentException("No spellcasting " + name));
	}
	...
	public int getLevel() {
		return getAttributes(CHARACTER_CLASS, CharacterClassLevel.class).mapToInt(CharacterClassLevel::getLevel).sum();
	}

	public int getLevel(CharacterClass characterClass) {
		return getAttributes(CHARACTER_CLASS, CharacterClassLevel.class)
				.filter(ccl -> ccl.hasCharacterClass(characterClass)).mapToInt(CharacterClassLevel::getLevel).sum();
	}
	...
	public Stream<CharacterClass> allowedMultiClasses() {
		return Arrays.stream(CharacterClass.values()).filter(cc -> cc.hasMulticlassPrerequisites(this))
	...
	private void generateChoices(CharacterClass characterClass) {
		getCharacterClassLevel(characterClass)
				.orElseThrow(() -> new IllegalStateException(
						"Generating choices for " + "non-existent class " + characterClass.toString()))
				.generateChoices(this);
		attributes.getAllAttributes().collect(toList()).forEach(attr -> attr.generateLevelChoices(this));
	}

	private Optional<CharacterClassLevel> getCharacterClassLevel(CharacterClass characterClass) {
		return getCharacterClassLevels().filter(ccl -> ccl.hasCharacterClass(characterClass)).findAny();
	}
	...
	public Stream<Attack> getAttacks() {
		return Stream.of(getUnarmouredAttacks(), getWeaponAttacks(), getSpellAttacks()).flatMap(Function.identity());
	}
	...
	private Stream<Attack> getUnarmouredAttacks() {
		...
		if (hasAttribute(MonkAbility.MARTIAL_ARTS)) {
			...
			damage = StringUtils.expand("[max($level 1:1d4,5:1d6,11:1d8,17:1d10)" + "bonus(max($str_mod,$dex_mod))]",
					this);
		}
		return Stream.of(new Attack(name, "5", getProficiencyBonus() + abilityBonus, damage, DamageType.BLUDGEONING));
	}
	...
	private Stream<Attack> getSpellAttacks() {
		return getAttributes(SPELL_ABILITY, SpellAbility.class).map(sa -> sa.getAttack(this))
	...
	public String getErrors() {
		return Stream.of(NAME, TRAIT, IDEAL, BOND, FLAW, PHYSICAL_DESCRIPTION, PERSONAL_HISTORY)
				.filter(at -> !hasAttribute(at) || getStringAttribute(at).isEmpty()).map(AttributeType::toString)
				.collect(joining(", "));
	}
